...
def main():
    with open('statistics', 'w') as outputFile:
        write_header_to_file(outputFile)
        ...
        for subdir in subdirs:
            ...
            for file in files:
                ...
                handle_comment_in_specific_file(fullFileName, outputFile)


def write_header_to_file(outputFile):
    header = "Page\tComment"
    print(header)
    outputFile.write(header + "\n")


def handle_comment_in_specific_file(fullFileName, outputFile):
    try:
...
def analyse_specific_file(full_filename):
    with open('statistics', 'w') as outputFile:
        write_header_to_file(outputFile)
        handle_comment_in_specific_file(full_filename, outputFile)
...
class MyHandler(FileSystemEventHandler):
    ...
    def on_modified(self, event):
        ...
        rejection_pattern = re.compile('.*collectedComments.pdf')
        res = pattern.match(event.src_path)
        if res:
            print("A pdf file was changed")
            if time.time() - self.last_change_time > 15:
                res = rejection_pattern.match(event.src_path)
                if res:
                    print("Rejected file to extract comments from based on its name.")
                else:
                    print("Analyzing the file")
                    analyse_specific_file(event.src_path)
                    print("Completed analysis of file")
                    self.last_change_time = time.time()
            else:
                print("Skipped file due to timing")
                print(self.last_change_time)
                print(time.time())
