...
This version of the mm exporter has been specifically designed for Freeplane.
http://freeplane.sourceforge.net/wiki/index.php/Main_Page
Freeplane is a fork of Freemind with many new features, some of them such as the
inclusion of LaTeX formulas are very useful for the purpose of this export filter.
...
Attribute features added by: Igor G. Olaizola
...
Modified_ 16th of July 2014: Compatibility issues with ##1.3.x## version of freeplane. [iolaizola 2.1]
...
See: https://sites.google.com/site/freemind2beamer/
https://github.com/igor-go/mm_xslt_exports
http://freeplane.sourceforge.net/wiki/index.php/Main_Page
-->
...
% !TEX encoding = latin1
\documentclass[usepdftitle=false,professionalfonts,compress,fleqn]{beamer}
...
<xsl:template name="itemization">
	...
	<!--Create variables to measure the depth and clean up the LaTeX code-->
	<xsl:variable name="depthlevel" select="(count(ancestor::node())-2)"/>
	<xsl:variable name="threshold" select="2"/>
	<xsl:variable name="bulletlevel" select="$depthlevel - $threshold"/>


	<xsl:if test="not(./cloud) and not(../cloud) and not(../../cloud)">
		<xsl:for-each select="(//node())[$bulletlevel >= position()]">
			<xsl:text>&#x9;</xsl:text>
		</xsl:for-each>
	...
	<xsl:for-each select="$i">
		<xsl:choose>
			...
			<xsl:when test="@TEXT and not(../cloud) and not(../../cloud)">  <!-- normal node case-->
				<xsl:for-each select="(//node())[$bulletlevel + 1  >= position()]">
					<xsl:text>&#x9;</xsl:text>
				</xsl:for-each>
				...
				<xsl:if test="current()/@FORMAT = 'latexPatternFormat'"> 
					<xsl:text>$&#xA;</xsl:text>
				</xsl:if>																									<!-- Equation filter for 1.3.x versions end-->
				...
				</xsl:if>																		<!-- Equation filter for 1.1.x and 1.2.x versions end-->
			</xsl:when>
			<xsl:when test="current()/richcontent/html/body/p/@text and not(../cloud) and not(../../cloud)">
				<xsl:text>3\item </xsl:text>
				<xsl:if test="current()/@FORMAT = 'latexPatternFormat'"> <!-- Equation filter for 1.3.x versions begin-->
					<xsl:text> $</xsl:text>
				</xsl:if>
				<xsl:call-template name="richtext"></xsl:call-template>
				...
				<xsl:if  test="current()/hook/@EQUATIION"> 							<!-- Equation filter for 1.3.x versions -->
					<xsl:text>&#xA;\begin{equation*}&#xA;&#x9;</xsl:text>
					<xsl:value-of select="current()/hook/@EQUATION" disable-output-escaping="yes"/>
				<xsl:text>&#xA;\end{equation*}</xsl:text>
				</xsl:if>																								<!-- Equation filter for 1.3.x versions end-->
